...
let auth,db,currentUser=null,currentProfile=null,currentUserRole=null,setupStep=1,profilePhotoFile=null,coverPhotoFile=null,profilePhotoCropped='',activeChatUid=null,activeChatRef=null,activeProfileUid=null;
...
function bindNavigation(){
 ...
 $('topProfile').onclick=()=>{activeProfileUid=currentUser?.uid||null;openPage('profilePage')}; $('mobileMenu').onclick=()=>{$('sidebar').classList.add('open');$('mobileOverlay').classList.remove('hidden')};$('mobileClose').onclick=closeMobile;$('mobileOverlay').onclick=closeMobile;
}
...
async function loadStories(){
  ...
  const now=Date.now();
  try{
    const [statusSnap,friendSnap,friendReverseSnap]=await Promise.all([
      db.ref('statuses').once('value'),
      db.ref('friendships/'+currentUser.uid).once('value'),
      db.ref('friendships').once('value')
    ]);
    const friendIds=new Set(Object.keys(friendSnap.val()||{}));
    // Compatibilidade: amizades antigas podem existir apenas no caminho do outro usuário.
    friendReverseSnap.forEach(userNode=>{
      if(userNode.child(currentUser.uid).exists() && userNode.child(currentUser.uid).val()!==false && userNode.child(currentUser.uid).val()!==null){
        friendIds.add(userNode.key);
      }
    });
...
async function loadRelationshipFriends(selectedUid=''){
  ...
  const snap=await db.ref('friendships/'+currentUser.uid).once('value');
  const ids=Object.keys(snap.val()||{}).filter(uid=>uid!==currentUser.uid);
  for(const uid of ids){const p=await getProfile(uid);const o=document.createElement('option');o.value=uid;o.textContent=(p.displayName||'Usuário')+' @'+(p.username||'');if(uid===selectedUid)o.selected=true;select.appendChild(o)}
}
function relationshipDisplay(p){
  const base=p.relationship||'Não informado';
...
async function getFriendshipState(uid){
 ...
 const [a,b]=await Promise.all([
   db.ref('friendships/'+currentUser.uid+'/'+uid).once('value'),
   db.ref('friendships/'+uid+'/'+currentUser.uid).once('value')
 ]);
 const isFriend=(a.exists()&&a.val()!==false&&a.val()!==null)||(b.exists()&&b.val()!==false&&b.val()!==null);
...
async function updateProfileFriendButton(){
 const a=$('profileAddFriend');if(!a)return;
 const state=await getFriendshipState(a.dataset.uid);
 a.disabled=false;a.classList.remove('danger');
...
function bindChat(){const open=$('openChatBar'),bar=$('chatFriendsBar'),close=$('closeChat'),dock=$('messageDock'),form=$('chatForm'),text=$('chatText');if(!open||!bar||!close||!dock||!form||!text)return;open.onclick=()=>{bar.classList.toggle('hidden');loadChatFriends()};close.onclick=()=>{dock.classList.add('hidden');if(activeChatRef)activeChatRef.off();activeChatRef=null};form.addEventListener('submit',async e=>{e.preventDefault();const t=text.value.trim();if(!t||!activeChatUid)return;const key=[currentUser.uid,activeChatUid].sort().join('_');await db.ref('messages/'+key).push({uid:currentUser.uid,text:t,createdAt:firebase.database.ServerValue.TIMESTAMP});text.value=''})}
