...
async function imageFileToSquareDataURL(file, options={}){
  if(!file) return '';
  ...
  const maxOutput=options.maxOutput||700*1024;
  ...
  const img=await new Promise((resolve,reject)=>{const el=new Image();el.onload=()=>resolve(el);el.onerror=()=>reject(new Error('Não foi possível processar a imagem.'));el.src=src});
  const side=Math.min(img.naturalWidth,img.naturalHeight);
  const sx=(img.naturalWidth-side)/2,sy=(img.naturalHeight-side)/2;
  const canvas=document.createElement('canvas');canvas.width=size;canvas.height=size;
  const ctx=canvas.getContext('2d');if(!ctx)throw new Error('Seu navegador não conseguiu processar a imagem.');
  ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
  ctx.drawImage(img,sx,sy,side,side,0,0,size,size);
  let q=quality,data=canvas.toDataURL('image/jpeg',q);
  while(data.length>maxOutput*1.37&&q>0.5){q-=0.06;data=canvas.toDataURL('image/jpeg',q)}
  return data;
}
...
async function changeUserRole(uid,role,select){
  if(await adminActorRole()!==2)throw new Error('Somente administradores podem alterar cargos.');
...
function openPage(id){
  ...
  document.querySelectorAll('#mainNav [data-page],#adminNavItem').forEach(b=>{
    const same=b.dataset.page===id;
    const settingsSub=b.dataset.settingsOpen;
    b.classList.toggle('active',same && (!settingsSub || settingsSub===window.__eclipseSettingsTab));
  });
  closeMobile();
...
async function loadProfilePosts(){
  const holder=$('profilePosts');if(!holder)return;
  const targetUid=$('profileAddFriend')?.dataset.uid||currentUser.uid;
  const pr=await getProfile(targetUid);
  const s=await db.ref('posts').orderByChild('uid').equalTo(targetUid).once('value');
  const a=[];s.forEach(x=>a.push({id:x.key,...x.val()}));a.sort((x,y)=>(y.createdAt||0)-(x.createdAt||0));
  holder.innerHTML=a.length?a.slice(0,10).map(p=>`<article class="profile-post">
    <div class="profile-post-head">${avatar(pr.photoURL,'mini-avatar')}<div><strong>${esc(pr.displayName||'Usuário')}</strong><small>@${esc(pr.username||'')} · ${p.createdAt?new Date(p.createdAt).toLocaleString('pt-BR'):''}</small></div></div>
    ${p.text?`<p class="profile-post-text">${esc(p.text)}</p>`:''}
    ${p.imageURL?`<img class="profile-post-image" src="${esc(p.imageURL)}" alt="Imagem da publicação">`:''}
    ${p.videoURL?`<video class="profile-post-video" src="${esc(p.videoURL)}" controls playsinline></video>`:''}
  </article>`).join(''):'<p class="muted">Nenhuma publicação ainda.</p>'
}
...
function showSettingsTab(tab){
  ...
  if(!tabs[tab])tab='account';
  window.__eclipseSettingsTab=tab;
  ...
  document.querySelectorAll('#mainNav [data-page="settingsPage"]').forEach(b=>b.classList.toggle('active',(!b.dataset.settingsOpen&&tab==='account')||(b.dataset.settingsOpen===tab)));
}

async function loadBlockedUsers(){
  const root=$('blockedUsersList');
  if(!root||!currentUser)return;
  root.classList.remove('hidden');root.innerHTML='<p class="muted">Carregando bloqueios...</p>';
  const snap=await db.ref('blocks/'+currentUser.uid).once('value');
  const ids=[];snap.forEach(x=>{if(x.val()===true||x.val())ids.push(x.key)});
  if(!ids.length){root.innerHTML='<p class="muted">Você não bloqueou ninguém.</p>';return}
  const rows=[];for(const uid of ids){const p=await getProfile(uid);rows.push(`<div class="blocked-user-row">${avatar(p.photoURL,'mini-avatar')}<div><strong>${esc(p.displayName||'Usuário')}</strong><small>@${esc(p.username||'')}</small></div><button class="secondary" data-unblock-user="${esc(uid)}">Desbloquear</button></div>`)}
  root.innerHTML=rows.join('');root.querySelectorAll('[data-unblock-user]').forEach(b=>b.onclick=async()=>{await db.ref('blocks/'+currentUser.uid+'/'+b.dataset.unblockUser).remove();await loadBlockedUsers()});
}
function bindSettingsTabs(){
  ...
  $('openBlockedUsers')?.addEventListener('click',loadBlockedUsers);
  $('settingsSearch')?.addEventListener('input',e=>{const q=e.target.value.trim().toLowerCase();document.querySelectorAll('.settings-menu-btn').forEach(b=>b.classList.toggle('hidden',!!q&&!b.textContent.toLowerCase().includes(q)))});
  showSettingsTab(window.__eclipseSettingsTab||'account');
}
